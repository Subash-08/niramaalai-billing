...
export async function invoicePdfBytes(s:State,b:Bill,templateId?:string){
 ...
 if(b.status==='Draft'||b.status==='Cancelled'){const watermarkText=b.status==='Draft'?'DRAFT · NOT ISSUED':'CANCELLED';for(let i=1;i<=doc.getNumberOfPages();i++){doc.setPage(i);doc.saveGraphicsState();doc.setTextColor(220,80,80);doc.setFontSize(36);doc.setFont('helvetica','bold');doc.text(watermarkText,width/2,doc.internal.pageSize.getHeight()/2,{align:'center',angle:45});doc.restoreGraphicsState();}}
...
export async function invoiceZipBytes(s:State,bills:Bill[],templateId?:string){if(!bills.length)throw new Error('Select at least one invoice.');if(bills.length>100)throw new Error('Export up to 100 invoices at a time.');const {default:JSZip}=await import('jszip');const zip=new JSZip();for(const b of bills){const bNum=(b as any).invoiceNumber||b.id;const bName=b.status==='Draft'?(String(bNum).startsWith('DRAFT')?String(bNum):`DRAFT_${bNum}`):String(bNum);zip.file(safeFilename(bName)+'.pdf',await invoicePdfBytes(s,b,templateId));}zip.file('invoice-index.xlsx',await workbookBytes('Invoice archive',['Invoice','Date','Customer','Total','Paid','Due'],bills.map(b=>[b.id,b.date,s.customers.find(c=>c.id===b.customerId)?.name||'',roundedTotal(b),paid(s,b.id),balance(s,b)]),'Selected invoice records'));return zip.generateAsync({type:'uint8array',compression:'DEFLATE'});}
